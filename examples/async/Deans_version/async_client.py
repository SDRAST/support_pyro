...
def square_callback(res):
    print("async_client: square handler got: {}".format(res))


def repeat_callback(res):
    print("async_client: repeat handler got: {}".format(res))

uri = "PYRO:BasicServer@localhost:9091"
proxy = async.AsyncProxy(uri)

print("repeat returns: {}".format(proxy.repeat("hello", 10, callback=repeat_callback)))
print("square returns: {}".format(proxy.square(2, callback=square_callback)))
print("simple returns: {}".format(proxy.simple('Hi!')))
